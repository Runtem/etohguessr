...
function extractInfoboxImage(wikitext) {
    ...
    const infoboxMatch = wikitext.match(
        /\{\{(?:[Tt]ower[Ii]nfobox)[^}]*?\|\s*image1\s*=\s*([^\n|}]+?)(?:\||\n|\}|$)/
    );
...
module.exports = async (req, res) => {
    try {
        ...
        const pages = categoryData.query?.categorymembers.filter(
            (page) =>
        ...
        const contentUrl = `${WIKI_API}?action=query&format=json&prop=revisions&rvprop=content&titles=${encodeURIComponent(
            randomPage.title
        )}`;
        ...
        if (imageName) {
            ...
            const imageUrlRes = await fetch(
                `${WIKI_API}?action=query&format=json&prop=imageinfo&iiprop=url&titles=${encodeURIComponent(
                    imageName
                )}`
            );
            if (imageUrlRes.ok) {
        ...
        if (!imageUrl) {
            const imageRes = await fetch(imageUrl, {
                headers: {
                    "User-Agent":
                        "Mozilla/5.0 (compatible; EToH-Wiki-Random-Tower/1.0)",
                    Referer: "https://jtoh.fandom.com/",
                    Origin: "https://jtoh.fandom.com",
                },
            });
            if (imageRes.ok) {
        ...
        return res.json({
            ...
            imageUrl,
            pageUrl: `https://jtoh.fandom.com/wiki/${encodeURIComponent(
                randomPage.title
            )}`,
        });
    } catch (error) {
